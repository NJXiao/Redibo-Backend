...
const { linkInsurancesToCar, InsuranceServiceError } = require('../../Par_2/services/insuranceService');
...
async function createFullCar(dto) {
  const {
    ...
    combustibles = [],
    caracteristicas = [],
    seguros = [], // Array de seguros adicionales { seguroId, fechaInicio, fechaFin }
  } = dto;
    ...
    const txResult = await prisma.$transaction(async (tx) => {
      ...
      const c = await tx.carro.create({
        ...
        include: {
          ...
          seguroCarro: { // --- MANTENIDO: Para que la respuesta final los incluya ---
            include: {
              seguro: true
            }
          }
      ...
      if (seguros && seguros.length > 0) {
        console.debug(`Calling insurance service to link ${seguros.length} insurances...`);
        await linkInsurancesToCar(tx, c.id, seguros); // Llama a la función importada
        console.debug(`Insurance linking process completed within transaction.`);
    } else {
        console.debug("No additional insurances provided to link.");
    }
      return { dir, c };
    });
